...
export class MainService extends BaseServiceService {
  ...
  checkPing() {
    return this.http.get<IMPing>(EMain.checkPing + `/${/* this.tg.getUserId() */312123}`   , { observe: 'response' })
      .pipe(
  ...
  getMain() {
    return forkJoin([
      ...
      this.http.get<IWathHistory[]>(EMain.history  + `/${/* this.tg.getUserId() */312123}`  )]
    ).pipe(
