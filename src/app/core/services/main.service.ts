...
export class MainService extends BaseServiceService {
  ...
  checkPing() {
    return this.http.get<IMPing>(EMain.checkPing + `/${/* this.tg.getUserId() */ 1805991962}`   , { observe: 'response' })
      .pipe(
  ...
  getMain() {
    return forkJoin([
      ...
      this.http.get<IWathHistory[]>(EMain.history  + `/${/* this.tg.getUserId() */ 1805991962}`  )]
    ).pipe(
